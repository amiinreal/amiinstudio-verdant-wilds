...
const Map := preload("res://Adventure/map.gd")
var session: Node3D
...
func _build_menu() -> void:
	...
	var intro: Label = _label("The world has forgotten its song.\nLearn its melodies. Grow the crossings.\nBring seven distant voices home.", 16, Color("b6c8b9"))
	box.add_child(intro)
...
func _build_game_ui() -> void:
	...
	_button("Songbook [J]", toggle_book, party_box)
	...
	quest_box.add_child(_label("YOUR NEXT VERSE", 11, Color("a4c498")))
	_quest = _label("", 16)
	_quest.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	_quest.custom_minimum_size.x = 365
	quest_box.add_child(_quest)
	_hint = _label("Q  Listen   E  Inspect / checkpoint\nR  Echo last phrase   H  Recall\nWASD  Move   Space  Jump   Esc  Menu", 12, Color("a7bcb0"))
...
func _build_book() -> void:
	...
	box.add_child(_label("The traveler's songbook", 27))
...
func _build_map() -> void:
	...
	box.add_child(_label("The seven voices", 27))
	_map = Control.new()
	_map.set_script(Map)
	_map.custom_minimum_size = Vector2(470, 430)
	box.add_child(_map)
	box.add_child(_label("1 Camp    2 Winds    3 Sunken Choir    4 Stonewake\n5 Lanternwood    6 Observatory    7 Heart\nGold: you  ·  Blue: friends  ·  Green: open crossings", 13, Color("b7cabb")))
...
func refreshed() -> void:
	if session != null and not session.state["solved"][6]:
		_completion_shown = false
	if session != null and session.state["solved"][6] and not _completion_shown:
...
func _refresh_book() -> void:
	if session == null:
		return
	var text_value: String = "Your notes change the world. Play them with keys 1–5.\nQ repeats the local shrine's clue. R creates a repeating Echo.\nFriends can contribute notes to the same phrase.\n\nLEARNED MELODIES\n"
	var effects: Array[String] = ["heal nearby travelers", "ride an updraft", "reveal hidden memories for 15s", "silence Discord for 8s"]
	for i in range(4):
		text_value += (Rules.SPELL_NAMES[i] + "   " + Rules.phrase_text(Rules.SPELLS[i]) + "   /   " + effects[i] if session.state["solved"][i] else "???   Restore " + Rules.NAMES[i]) + "\n"
	text_value += "\nTHE SEVEN VOICES\n"
	for i in range(7):
		text_value += ("✓  " if session.state["solved"][i] else "○  ") + Rules.NAMES[i] + "\n"
	text_value += "\nEcho Observatory: play its four notes, then press R.\nOnline: one friend can start the phrase and another can finish it.\nMemories are shared; checkpoint shrines restore your health."
...
func _process(delta: float) -> void:
	...
	if body == null:
		return
	var island: int = session.world.nearest_island(Vector2(body.position.x, body.position.z))
	_region.text = Rules.NAMES[island]
	_stats.text = "HEALTH %d   ·   %d / 7 VOICES   ·   %d MEMORIES" % [body.health, session.state["solved"].count(true), session.state["shards"]]
	_party.text = session.status + "\n%d / 6 travelers  ·  Expedition %d" % [session.players.size(), session.state["expedition"]]
	var target: int = 0
	for i in range(7):
		if not session.state["solved"][i] and Rules.prerequisites(i, session.state["solved"]):
			target = i
			break
	var site: int = session.nearest_site(session.local_id())
	if site >= 0 and not session.state["solved"][site]:
		_quest.text = Rules.DESCRIPTIONS[site]
		var phrase: Array = Rules.required_phrase(session.state["seed"], site)
		_phrase.text = ("REVERSE THE RUNES  " if site == 1 else "SHRINE PHRASE  ") + Rules.phrase_text(Rules.melodies(session.state["seed"])[site]) + "   [%d/%d]" % [session.progress[site], phrase.size()]
	else:
		var distance: int = int(body.position.distance_to(session.world.shrine_position(target)))
		_quest.text = "Reach %s\n%d m away · follow open crossings on the map [M]." % [Rules.NAMES[target], distance]
		_phrase.text = "PLAY THE WORLD  ·  keys 1–5  ·  songs in [J]"
	_map.queue_redraw()
