...
func generate(seed_value: int) -> void:
	...
	var cached: Variant=preload("res://Adventure/generation_cache.gd").read_data("terrain",seed_value,landscape_version)
	if cached is PackedFloat32Array and cached.size()==66049:
		heights=cached; base_heights=heights.duplicate(); return
	heights.resize((RES + 1) * (RES + 1))
	for z in range(RES + 1):
		if z%32==0: preload("res://Adventure/loading_screen.gd").show_progress(8+float(z)/256*12,"Shaping hills and rivers…")
		for x in range(RES + 1):
			heights[z * (RES + 1) + x] = raw_height(Vector2(x, z) * 4 - Vector2.ONE * 512)
	_grade_routes()
	base_heights=heights.duplicate()
	preload("res://Adventure/generation_cache.gd").write_data("terrain",seed_value,landscape_version,heights)
...
func _grade_routes() -> void:
	...
	for z in range(RES+1):
		if z%32==0: preload("res://Adventure/loading_screen.gd").show_progress(20+float(z)/256*15,"Smoothing roads and trails…")
		for x in range(RES+1):
			var p: Vector2=Vector2(x,z)*4-Vector2.ONE*512
			# Grading only affects a 12 m strip; skip distant terrain before costly sampled profiles.
			if road_distance(p)>12: continue
...
func base_height(p: Vector2) -> float:
	...
	if landscape_version>=2:
		# Broad woodland bowls, rolling meadows, and a broken mountain ridge.
		var woodland: float=exp(-pow((p.x+185)/130,2)-pow((p.y-35)/165,2))
		h+=woodland*(sin(p.x*.035)*cos(p.y*.027)*5.0-3.0)
		var hills: float=exp(-pow((p.x-220)/150,2)-pow((p.y-100)/175,2))
		h+=hills*(sin(p.x*.024+p.y*.019)*7+cos(p.y*.041)*3)
		var ridge: float=exp(-pow((p.x-170)/135,2)-pow((p.y+290)/110,2))
		h+=ridge*absf(sin(p.x*.035+p.y*.012))*16
	return h
